...
class Encoder(nn.Module):
    def __init__(self, input_size, embed_size, device='cuda'):
    ...
    def forward(self, x):
        ...
        x_embed = F.dropout(F.elu(self.fc(x_w)), p=0.3,
...
class CNN(NNet):
    def __init__(self, input_size, embed_size, num_classes, device):
        super(NNet, self).__init__()
        self.embed_size = embed_size
        self.num_classes = num_classes
    ...
    def forward(self, x):
        # Set initial hidden and cell states
        batch_size = x.size(0)
        embed_1 = self.encoder1(x)
        embed_2 = self.encoder2(x)
        # Decode the hidden state of the last time step
        out1 = F.dropout(F.elu(self.fc1(embed_1)),
                         p=0.3, training=self.training)
        out2 = F.dropout(F.elu(self.fc2(embed_2)),
                         p=0.3, training=self.training)
        out = torch.cat([self.fc3(out1), self.fc4(out2)], dim=1)
        return torch.sigmoid(out)
